...
class Uci(collections.namedtuple("Entry", ["key", "raw_move", "weight", "learn"])):
    """An entry from a polyglot opening book."""
    # la formula es 8*fila+columna
    """
    ...
    promocion_piezas = " nbrq"

    def move(self):
        """Obtiene el movimiento como una string formato uci."""
        ...
        promotion = promotion_part + 1 if promotion_part else 0
        
        jugada = self.casillas[from_square] + self.casillas[to_square]
        ...
        #if promotion != 0:
        return jugada + self.promocion_piezas[promotion]
        #else:
...
class MemoryMappedReader(object):
    """Mapea un libro de aperturas polyglot a la memoria"""
...
class ClaveHash:
    ...
    # La clave / "key" es un hash de 64 bit de una posición en el tablero. 
    # Su diseño se explica a continuación

    # El array Random64 es, de hecho una concatenacón de 4 subarrays:
    #   RandomPiece     (offset:   0, longitud: 768)
    #   RandomCastle    (offset: 768, longitud:   4) 
    #   RandomEnPassant (offset: 772, longitud:   8)
    #   RandomTurn      (offset: 780, longitud:   1)

    # Pieza
    """
    "pieza" es el OR exclusivo de las entradas de RandomPiece, 
    una para cada pieza en el tablero. 
    El offset de la entrada en RandomPiece correspondiente a
    una pieza se computa como sigue.

    offset_piece=64*kind_of_piece+8*row+file;

    "row" y "file" se cuentan del 0 al 7. Con esta convención tenemos:
    ...
    "kind_of_piece" se computa como sigue
    ...
    # Enroque
    """
    "castle" es el OR exclusivo de las entradas de RandomCastle. 
    Los offsets se computan como sigue.

    blanco puede enrocar corto     0
    blanco puede enrocar largo     1
    negro puede enrocar corto      2
    negro puede enrocar largo      3

    Si no se puede aplicar ninguno de estos flags se puede aplicar,
    entonces  castle=0.

    Notar que estos flags representan un 'posible futuro enroque'
    como se especifica en la FEN, incluso si tal enroque no es ahora 
    mismo posible debido a piezas que bloquean o ataques del enemigo.

    """

    # Al Paso
    """
    Si el oponente ha realizado un avance de dos casillas con el peón y 
    ahora hay un peón al lado que pertenece al jugador wue le toca para mover, 
    entonces "enpassant" es la entrada de RandomEnPassant cuyo offset es 
    la columna del peón avanzado (contado desde 0(= a) a 7(= h)). 
    Si esto no se aplica entonces enpassant = 0.
    ...
    Also note that it is irrelevant if the potential en passant capturing move is legal or 
    not (examples where it would not be legal are when the capturing pawn is pinned or when 
    the double pawn push was a discovered check). 
    """


    RandomPiece     = Random64
    RandomCastle    = [Random64[768], Random64[769], Random64[770], Random64[771]]
    RandomEnPassant = [Random64[772], Random64[773], Random64[774], Random64[775], \
    ...
    def hash(self, fen):
        ...
        board_s        = fen_arr[0]
        to_move_c      = fen_arr[1]
        castle_flags_s = fen_arr[2]
        ep_square_s    = fen_arr[3]
        ...
        while True:
            ...
            if c=='/':
                ...
                f=0
                continue
            if (ord('1') <=ord(c)) and (ord('8') >= ord(c)):
        ...
        for f in range(8):
            for r in range(8):
                c=board[f][r]
                if(c!='-'):
                    p_enc = self.piece_names.find(c)
                    key^=self.RandomPiece[64*p_enc+8*r+f]
        
        # los enroques
        p=0
        while True:
            if p>=len(castle_flags_s):
                break
            c=castle_flags_s[p]
            p += 1
        ...
        if ep_square_s[0]!='-':
            ...
            if to_move_c=='b':
                if (f>0 and board[f-1][3]=='p')or \
                        (f<7 and board[f+1][3]=='p'):
                    key^=self.RandomEnPassant[f]
            else:
                if((f>0 and board[f-1][4]=='P')or \
                        (f<7 and board[f+1][4]=='P')):
                    key^=self.RandomEnPassant[f]
...
def busca_clave(v_fen):
    clave = ClaveHash()
    # print(562, clave.hash(v_fen))
    return clave.hash(v_fen)


def busca_libro(libro_bin, v_fen):
    total_weight = 0
    lista_jugadas = []
    #clave = ClaveHash()
    key = busca_clave(v_fen)    #clave.hash(v_fen)
    # print("linea 558", key)
    with open_reader(libro_bin) as reader:
        for entry in reader.find_all(key):
            total_weight += entry.weight
            lista_jugadas.append({'movim':entry.move(), 'peso': entry.weight})
            #  print(entry.move(), entry.weight, entry.learn)
            
    for entrada in lista_jugadas:
        print("%s --> %.2f %%" % (entrada['movim'], (entrada['peso'] * 100)/total_weight))
    
    
if __name__ == '__main__':
    #Creamos un lector para el fichero en el directorio dado.
    """
    ...
    key=0756b94461c50fb0  / decimal = 528813709611831216
    ...
    """
    """
    key=0x2d3888dac361814a
    with open_reader("libro.bin") as reader:
        for entry in reader.find_all(key):
            print(entry.move(), entry.weight, entry.learn)
    """
    
    busca_libro("book.bin", "rn2k2r/pp2bppp/1qp2n2/3p1b2/8/3P1NP1/PPQ1PPBP/RNB2RK1 b kq - 4 9")
